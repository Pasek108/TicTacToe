from flask import Flask, render_template, request
from flask_socketio import SocketIO, join_room, leave_room

import math
import json

from Game import Game

app = Flask(__name__)
app.config['SECRET_KEY'] = 'vnkdjnfjknfl1232#'
socketio = SocketIO(app)
rooms = []


# ------------------ routes ------------------
@app.route("/")
def index():
    return render_template('index.html')


@app.route("/multiplayer")
def multiplayer():
    return render_template('multiplayer.html')


# ------------------ sockets ------------------
@socketio.on("disconnect")
def on_disconnect():
    for room in rooms:
        if room != "" and request.sid in rooms[int(room.id)].players:
            on_leave_game(room.id)


@socketio.on("leave_game")
def on_leave_game(room_id):
    leave_room(str(room_id))

    if not rooms[int(room_id)].player_leave(request.sid):
        rooms[int(room_id)] = ""

    socketio.emit("refresh_lobby", on_load_lobby())


@socketio.on("load_lobby")
def on_load_lobby():
    rooms_info = map(lambda room: "" if room == "" else json.dumps(room.get_game_info(request.sid)), rooms)
    return list(rooms_info)


@socketio.on("create_game")
def on_create_game(data):
    game_info = json.loads(data)

    game_id = -1
    for i in range(len(rooms)):
        if rooms[i] == "":
            game_id = i
            break

    if game_id < 0:
        game_id = len(rooms)

    game = Game(game_id,
                game_info["gamemode_id"],
                int(game_info["reverse"]),
                int(game_info['starting_player_id']),
                3 + math.floor(game_info["gamemode_id"] / 2),
                game_info["password"],
                game_info["change"],
                request.sid)

    if game_id < len(rooms):
        rooms[game_id] = game
    elif game_id == len(rooms):
        rooms.append(game)

    socketio.emit("refresh_lobby", on_load_lobby())
    join_room(str(game_id))
    return rooms[int(game_id)].get_game_info(request.sid)


@socketio.on("join_room")
def on_join_room(room_id):
    if not rooms[int(room_id)].player_join(request.sid):
        return False

    join_room(str(room_id))
    socketio.emit("refresh_lobby", on_load_lobby())
    return rooms[int(room_id)].get_game_info(request.sid)


@socketio.on("move")
def on_move(data):
    if not rooms[data['room']].player_move(data['x'], data['y'], data['player_start_id']):
        return

    socketio.emit("player_move", data, room=data['room'])


if __name__ == '__main__':
    socketio.run(app, debug=True, allow_unsafe_werkzeug=True)
