  ...
  availableMoves() {
    ...
    for (let i = 0; i < this.size; i++) {
      for (let j = 0; j < this.size; j++) {
        if (this.moves_counter < 6 && this.game_state[i][j] === "") moves.push([[-1, -1], [i, j] ]);
        else if (this.moves_counter >= 6 && this.game_state[i][j] === this.options[this.current_player_id]) {
          if (this.game_state[i]?.[j - 1] === "") moves.push([[i, j], [i, j - 1]]);
          if (this.game_state[i]?.[j + 1] === "") moves.push([[i, j], [i, j + 1]]);
          if (this.game_state[i - 1]?.[j] === "") moves.push([[i, j], [i - 1, j]]);
          if (this.game_state[i + 1]?.[j] === "") moves.push([[i, j], [i + 1, j]]);
        }
      }
    }
